...
parser.add_argument("-results_path", help="results_path", default="./results_parallel/")
parser.add_argument("-e_goal", type=float, help="epsilon goal", default = 0.01)
parser.add_argument("-conf", type=float, help="confidence", default = 0.99)
parser.add_argument("-p", type=float, help="p", default = 0.5)
parser.add_argument("-seed", type=int, help="seed", default=0)

def worker_indices(worker_id, model_args, fault_indices, golden_preds, baseline_hist, baseline_dist, num_classes, return_dict):
    """
    Worker che processa una lista di fault_indices.
    """
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = get_network(model_args["net"], load_path=model_args["weights_path"])
    model.to(device)
    model.eval()
    ...
    for inj_id, fault_site in fault_indices:
        inj_cnt+=1
        if inj_cnt % 1000 == 0:
            print(f"[Worker {worker_id}] Processed {inj_cnt}/{len(fault_indices)} faults")
        frcrit, fault, bias, fh, mbc, cbc, cm = _evaluate_faulty_model(
            model, device, model_args["test_loader"], model_args["clean_by_batch"], injector,
            fault_site, inj_id, total_samples, baseline_hist, baseline_dist, num_classes
        )
    ...
    return_dict[worker_id] = {
        "sum_fr": sum_fr,
        "n_inj": n_inj,
        "global_fault_hist": global_fault_hist,
        "mism_by_clean_sum": mism_by_clean_sum,
        "cnt_by_clean_sum": cnt_by_clean_sum,
        "global_cm_cf": global_cm_cf,
        "maj_shares": maj_shares,
        "kls": kls,
        "mean_frcrit": mean_frcrit,
        "m2_frcrit": m2_frcrit,
        "count_fr": count_fr,
        "top_heap": top_heap
    }
...
def main_parallel_save(args, num_processes=4):
    ...
    layerwise_name = f'_layer_{layer_name}'
    nome_file = f"one_step_net_{network_name}_data_{dataset_name}_egoal_{e_goal}{'' if layer_name==None else layerwise_name}.txt"
    os.makedirs(args.results_path, exist_ok=True)
    output_file = os.path.join(args.results_path, nome_file)    
    
    indexer = FaultIndexer(model, layer_name=layer_name)
    fault_sites = indexer
    ...
    total_possible, use_fpc_auto, _, n_inf, n_fpc, ratio = \
        decide_sampling_policy(num_faults, K, e_goal, conf, p, exhaustive_cap)
    ...
    pilot = plan_n_with_fpc(
            p_hat=p, e_target=e_goal, conf=conf,
    ...
    # --- Genera lista completa di indici da testare ---
    if use_wor:
        fault_indices = []
        injected = 0
        inj_id = 0
        while injected < pilot:
            ...
            inj_id += 1
            fault_indices.append((inj_id, fault_site))
    else:
        fault_indices = list([(idx, fs) for idx, fs in enumerate(gen)])
    print(len(fault_indices))
    # --- Dividi lista per processi ---
    chunk_size = math.ceil(len(fault_indices)/num_processes)
    chunks = [fault_indices[i*chunk_size:min((i+1)*chunk_size,len(fault_indices))] for i in range(num_processes)]
    ...
    for i, chunk in enumerate(chunks):
        p = mp.Process(target=worker_indices, args=(i, model_args, chunk, golden_preds, baseline_hist, baseline_dist, num_classes, return_dict))
        p.start()
        processes.append(p)
    for p in processes:
        p.join()
    ...
    for worker_res in return_dict.values():
        count_fr += worker_res["count_fr"]
        sum_fr += worker_res["sum_fr"]
        n_inj += worker_res["n_inj"]
        global_fault_hist += worker_res["global_fault_hist"]
        mism_by_clean_sum += worker_res["mism_by_clean_sum"]
        cnt_by_clean_sum  += worker_res["cnt_by_clean_sum"]
        global_cm_cf      += worker_res["global_cm_cf"]
        maj_shares.extend(worker_res["maj_shares"])
        kls.extend(worker_res["kls"])

        # Aggiornamento online media e varianza
        n2 = worker_res["count_fr"]
        mean2 = worker_res["mean_frcrit"]
        m22 = worker_res["m2_frcrit"]
        ...
        if count_fr == 0:
            ...
            delta = mean2 - mean_frcrit
            n = count_fr + n2
            mean_frcrit = mean_frcrit + delta * n2 / n
            m2_frcrit = m2_frcrit + m22 + delta**2 * count_fr * n2 / n
            count_fr = n

        for item in worker_res["top_heap"]:
            if len(top_heap) < 100:
    ...
    # --- Salvataggio file identico ---
    layerwise_name = f'_layer_{args.layer_name}' if args.layer_name else ''
    nome_file = f"one_step_net_{args.net}_data_{args.data}_egoal_{args.e_goal}{layerwise_name}.txt"
    os.makedirs(args.results_path, exist_ok=True)
    output_file = os.path.join(args.results_path,nome_file)
    
    with open(output_file, "w") as f:
        f.write(f"[PROP/{ep_control}] K={K}  FRcrit_avg={avg_frcrit:.8f}  conf={conf}  n={n_inj}\n")
        ...
        f.write(f"injections_used={n_inj}  pilot={pilot}\n")
        ...
        f.write(f"Top-{min(100, len(top_sorted))} worst injections\n")
    ...
    print(f"[PROP/{ep_control}] saved {output_file} – {dt_min:.2f} min "
          f"(avg FRcrit={avg_frcrit:.6f}, half_norm={half_norm:.6f}, n={n_inj})")
...
if __name__ == "__main__":
    ...
    main_parallel_save(args, num_processes=10)