...
parser.add_argument("-e_start", type=float, help="epsilon start", default = 0.05)
...
parser.add_argument("-p0", type=float, help="p0", default = 0.5)
...
def main(args):
    ...
    nome_file = f"iterative_fi_net_{network_name}_data_{dataset_name}_estart_{e_start}_egoal_{e_goal}{'' if layer_name==None else layerwise_name}.txt"
    ...
    pilot = plan_n_with_fpc(
            p_hat=p0, e_target=e_start, conf=conf,
    ...
    with open(output_file, "w") as f:
        ...
        f.write(f"injections_used={n_inj}  pilot={pilot}  budget_cap={budget_cap}\n")
